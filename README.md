...
- **Layout Builder**: Enable the core Layout Builder module to allow drag-and-drop page layouts using the custom two-column layout provided by the theme.
- **Sync Files from EFS**: Use `scripts/sync-efs.sh` to copy media between environments.
...
- **Security**:
    - Use modules like `login_security`, `seckit`, `honeypot`, and `captcha` for enhanced protection. If preferred, the security headers generated by the Seckit module can be exported to a custom settings file instead of relying on the module.
