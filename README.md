...
- **EFS (Elastic File System)**: Shared file storage for Drupal's files directory. Use `scripts/sync-efs.sh` to move files between environments.
...
- **Sync Files with EFS**: Use `scripts/sync-efs.sh <source> <dest>` to copy files between your local EFS mount and other environments.
