...
final class ScheduledTransitionTest extends KernelTestBase {
  ...
  public function testScheduledRevision(): void {
    ...
    $this->assertEquals('Processed scheduled transition #1', $logs[1]['message']);
  ...
  public function testScheduledRevisionRecreateNonDefaultHead(): void {
    ...
    $this->assertEquals('Processed scheduled transition #1', $logs[2]['message']);
    $this->assertEquals('Deleted scheduled transition #1', $logs[3]['message']);
  ...
  public function testScheduledTransitionStateAfterProcessing(): void {
    ...
    $createScheduledTransition = static function (TestEntity $entity) use ($workflow): ScheduledTransitionInterface {
      $scheduledTransition = ScheduledTransition::createFrom(
        $workflow,
        'published',
        $entity,
    ...
    $this->assertNotNull($scheduledTransition);
    $this->assertTrue($scheduledTransition->isProcessed());
    $entity = $entity::load($entity->id());
    $this->assertEquals([$entity->getRevisionId()], $scheduledTransition->getProcessedRevisions());
  }
